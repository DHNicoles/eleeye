...
void PipeStruct::Open(const char *szProcFile) {
  ...
  if (szProcFile == NULL) {
    ...
    GetCurrentDirectory(PATH_MAX_CHAR, LPWSTR(szCurDir));
    ParseDir(szDir, szProcFile);
    SetCurrentDirectory(LPCWSTR(szDir));
    ...
    if (CreateProcess(NULL, (LPWSTR) szProcFile, NULL, NULL, TRUE, DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP | IDLE_PRIORITY_CLASS, NULL, NULL, &si, &pi)) {
    ...
    SetCurrentDirectory(LPCWSTR(szCurDir));
  }
  if (bConsole) {
